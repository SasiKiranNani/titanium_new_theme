...
<body>
    ...
    <div class="content" style="max-height: 410px; height: 100%; margin-bottom: 80px;">
        @php
        $hasServiceJobs = $serviceJobs->isNotEmpty();
        $hasMiscellaneous = $miscellaneousItems->isNotEmpty();
    @endphp

    @if ($hasServiceJobs || $hasMiscellaneous)
        @php
            $totalServiceJobs = $hasServiceJobs ? $serviceJobs->sum('price') : 0;
            $totalMiscellaneous = $hasMiscellaneous ? $miscellaneousItems->sum('total_price') : 0;
        @endphp

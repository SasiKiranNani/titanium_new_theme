                        ...
                        <tbody>
                            @if ($upcomingVehicles->isNotEmpty())
                                @foreach ($upcomingVehicles as $vehicle)
                                    <tr>
                                        <td>{{ request('sort_order') == 'desc' ? $upcomingVehicles->total() - (($upcomingVehicles->currentPage() - 1) * $upcomingVehicles->perPage() + $loop->iteration - 1) : ($upcomingVehicles->currentPage() - 1) * $upcomingVehicles->perPage() + $loop->iteration }}
                                        </td>
                        ...
                        <div class="dt-paging">
                            @if (isset($perPage) && $perPage !== 'all')
                                <div class="pagination-container">
                                    {{  $upcomingVehicles->appends(request()->query())->onEachSide(1)->links('pagination::bootstrap-4') }}
                                </div>
                            @endif
                            <!-- Add this after the bootstrap pagination -->
                            @if (isset($perPage) && $perPage !== 'all')
                                <div class="simple-tailwind-pagination hidden">
                                    {{  $upcomingVehicles->appends(request()->query())->onEachSide(1)->links('pagination::simple-tailwind') }}
                                </div>
                            @endif
                        </div>
                    </div>

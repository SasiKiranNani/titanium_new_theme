                        ...
                        <tbody>
                            @if ($pastVehicles->isNotEmpty())
                                @foreach ($pastVehicles as $vehicle)
                                    <tr>
                                        <td>{{ request('sort_order') == 'desc' ? $pastVehicles->total() - (($pastVehicles->currentPage() - 1) * $pastVehicles->perPage() + $loop->iteration - 1) : ($pastVehicles->currentPage() - 1) * $pastVehicles->perPage() + $loop->iteration }}
                                        </td>
                        ...
                        <div class="dt-paging">
                            @if (isset($perPage) && $perPage !== 'all')
                                <div class="pagination-container">
                                    {{  $pastVehicles->appends(request()->query())->onEachSide(1)->links('pagination::bootstrap-4') }}
                                </div>
                            @endif
                            <!-- Add this after the bootstrap pagination -->
                            @if (isset($perPage) && $perPage !== 'all')
                                <div class="simple-tailwind-pagination hidden">
                                    {{  $pastVehicles->appends(request()->query())->onEachSide(1)->links('pagination::simple-tailwind') }}
                                </div>
                            @endif
                        </div>
                    </div>
